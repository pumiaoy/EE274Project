...
# Inputs:
time_window = 361  #360 # number of time samples in data in seconds
...
for batch_num in range(run1.num_batches):
    ...
    frame = 1001
    plt.imshow(data[:,:,frame], cmap = plt.cm.gray)
    plt.colorbar()
    plt.title("Frame 1001 of Video")
    plt.show()
    plt.savefig( "/home/pumiaoy/stanford_compression_library/Project/frame1001.jpg")
...
size = 16, 32
duration = 32*(number_batch-1)
fps = 32

import skvideo.io

#imageio.mimwrite('/home/pumiaoy/stanford_compression_library/Project/Raw_video.mp4', data , fps = float(20))
writer = skvideo.io.FFmpegWriter("/home/pumiaoy/stanford_compression_library/Project/6Minute_video.mp4")
for frame in range(fps * duration):
