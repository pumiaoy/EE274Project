...
ret = True
while ret:
    ret, img = cap_1.read() # read one frame from the 'capture' object; img is (H, W, C)
    if ret:
        gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
plt.plot(video_comp[4000:7000,10,27])
plt.plot(video_raw[4000:7000,10,27])
plt.legend(["Before","After"])
plt.title("Before and After H265 Video Compression crf=28")
plt.show()
plt.savefig( "/home/pumiaoy/stanford_compression_library/Project/BeforeAfter.jpg")

print(np.shape(video_comp))
